...
#SURF
\${PLINK_PATH} --bfile \${EUR_1KG_BFILE} --clump-p1 1 --clump-p2 1 --clump-r2 0.2 --clump-kb 250 --clump \${SUMSTATS} --clump-snp-field SNP --clump-field P \
        --extract \${SURF_PATH}/1000G_phase3_master_scores_1KG-pruned_quantile_normalized_chrALL.GENERIC.\${SLURM_ARRAY_TASK_ID}.SNPs --out \${RESULTS_PATH}/SURF/\${TRAIT}.\${ANCESTRY}.SURF.\${SLURM_ARRAY_TASK_ID}.clump
...
#TURF
\${PLINK_PATH} --bfile \${EUR_1KG_BFILE} --clump-p1 1 --clump-p2 1 --clump-r2 0.2 --clump-kb 250 --clump \${SUMSTATS} --clump-snp-field SNP --clump-field P \
        --extract \${TURF_PATH}/1000G_phase3_master_scores_1KG-pruned_quantile_normalized_chrALL.\${TISSUE}.\${SLURM_ARRAY_TASK_ID}.SNPs --out \${RESULTS_PATH}/TURF/\${TRAIT}.\${ANCESTRY}.TURF.\${SLURM_ARRAY_TASK_ID}.clump

\${PLINK2_PATH} --pgen \${BED_PATH} --pvar \${BIM_PATH} --psam \${FAM_PATH} --score \${SUMSTATS} 1 2 3 header no-mean-imputation --q-score-range \${RANGE_LIST} \${SUMSTATS}.SNP.pvalue \
        --extract \${RESULTS_PATH}/TURF/\${TRAIT}.\${ANCESTRY}.TURF.\${SLURM_ARRAY_TASK_ID}.clump.clumped --keep \${SAMPLES_PATH} --out \${RESULTS_PATH}/SURF/\${TRAIT}.\${ANCESTRY}.TURF.\${SLURM_ARRAY_TASK_ID}
